  ...
  fetch(`/animes/${animeId}`)
    ...
    .then((anime) => {
      ...
      document.getElementById('anime-rating').textContent =
        `${anime.avgRating?.toFixed(1) ?? '0.0'} of ${anime.totalRatings}`;
      ...
      document.getElementById('anime-release-date').textContent = new Date(
        anime.date_aired,
      ...
      document.getElementById('anime-scores').textContent =
        `${anime.avgRating?.toFixed(1) ?? '0.0'} ★`;
